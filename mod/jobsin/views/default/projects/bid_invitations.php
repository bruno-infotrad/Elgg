...
$min_rate = elgg_extract("min_rate", $vars);
...
foreach ($group_bids as $group_bid) {
	...
	$body .= '</div>';
	if ($group_bid->status == 'pending' && $min_rate) {
		$body .= '<div class="task-rate" id="lowest-rate">';
		$body .= '<p>'.elgg_echo('task:lowest_rate').'</p>';
		$body .= '<p>'.elgg_view('input/text',array('maxlength' => 5, 'name' => 'rate', 'value' => $min_rate,'readonly' => true)).'</p>';
		$body .= '</div>';
	}
